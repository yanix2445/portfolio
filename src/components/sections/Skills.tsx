...
import { useEffect, useState, FC, createContext, useContext, ReactNode } from "react";
...
export function SkillCategory({ name, icon: Icon, skills, tools }: { name: string, icon: any, skills: string[], tools: Tool[] }) {
    return (
        <div className="flex flex-col gap-4 border-l border-white/10 pl-6 group [content-visibility:auto]">
            <div className="flex items-center gap-2 mb-1">
                <Icon className="w-4 h-4 text-[#CC9400] opacity-70" />
            ...
            <div className="flex flex-wrap gap-4 mt-3">
                {tools.map((t, i) => (
                    <div key={i} className="group/tool relative flex items-center justify-center">
                        <t.icon
                            className="w-5 h-5 transition-[filter,opacity] duration-300 opacity-100 grayscale-0 md:opacity-50 md:grayscale group-hover:opacity-100 group-hover:grayscale-0"
                            style={{ color: t.color }}
                            aria-label={t.name}
                        />
                    </div>
...
export default function SkillsSection() {
    return (
        <SkillsRoot.Provider>
            <SkillsRoot>
                <SkillsRoot.Header title="Competencies" />
